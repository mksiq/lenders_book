...
public class BorrowerResource {
	...
	public ResponseEntity<List<Borrower>> findAll(){
		List<Borrower> list = service.findAll();
	...
	public ResponseEntity<Void> insert(@Valid @RequestBody BorrowerDTO objDto){
		Borrower obj = service.fromDTO(objDto);
	...
	public ResponseEntity<Void> update(@Valid @RequestBody BorrowerDTO objDto, @PathVariable Integer id){
		Borrower obj = service.fromDTO(objDto);
		obj.setId(id);
